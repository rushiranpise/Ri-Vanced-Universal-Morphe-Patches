...
public abstract class Setting<T> {
     ...
     */
    public static final SharedPrefCategory preferences = new SharedPrefCategory("rivanced_prefs");
     ...
     */
    private static final String OPTIONAL_RIVANCED_SETTINGS_PREFIX = "rivanced_";

    /**
     * The path, minus any 'rivanced' prefix to keep json concise.
     */
    private String getImportExportKey() {
        if (key.startsWith(OPTIONAL_RIVANCED_SETTINGS_PREFIX)) {
            return key.substring(OPTIONAL_RIVANCED_SETTINGS_PREFIX.length());
        }
        return key;
    }
    ...
    public static boolean importFromJSON(Context alertDialogContext, String settingsJsonString) {
        try {
            ...
            Utils.runOnMainThreadDelayed(() -> Utils.showToastLong(numberOfSettingsImportedFinal == 0
                            ? str("rivanced_settings_import_reset")
                            : str("rivanced_settings_import_success", numberOfSettingsImportedFinal)),
                    150);

            return rebootSettingChanged;
        } catch (JSONException | IllegalArgumentException ex) {
            Utils.showToastLong(str("rivanced_settings_import_failure_parse", ex.getMessage()));
            Logger.printInfo(() -> "", ex);
        } catch (Exception ex) {
